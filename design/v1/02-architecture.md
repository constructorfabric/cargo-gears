...
- **`codegen`** owns the generation of `.gears/<name>/` projects. Isolating generation makes it testable with
...
```text
1. Discover manifest (Gears.toml or --manifest)
...
5. Generate server project (.gears/<app>-<env>/)
...
- **Environment setup**: sets `GEARS_CONFIG`, `RUSTFLAGS`, and other required variables.
...
is sufficient for the Gears team's needs and avoids the complexity of a plugin API before usage patterns are
established.
