    ...
    pub fn run(&self) -> anyhow::Result<()> {
        self.path_config
            .with_workspace_dir(|workspace_path, config_path| {
                let local_modules = get_module_name_from_crate(Some(workspace_path))?;
                let config = load_config(config_path)?;
                let enabled_modules: BTreeSet<_> =
                    config.modules.keys().map(String::as_str).collect();
