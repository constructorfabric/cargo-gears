mod common;

use common::{assert_parse_error, parse_command};
use cyberware_cli_core::CyberfabricCommand;
use cyberware_cli_core::help::{HelpArgs, HelpCommand, SchemaArgs, SchemaTarget, Topic, TopicArgs};
use std::path::PathBuf;

#[test]
fn parses_help_schema_manifest() {
    let command = parse_command(&["cyberfabric", "help", "schema", "manifest"]);

    assert_eq!(
        command,
        CyberfabricCommand::Help(HelpArgs {
            command: HelpCommand::Schema(SchemaArgs {
                target: SchemaTarget::Manifest,
                section: None,
            }),
        })
    );
}

#[test]
fn parses_help_schema_config_with_section() {
    let command = parse_command(&[
        "cyberfabric",
        "help",
        "schema",
        "config",
        "--section",
        "database",
    ]);

    assert_eq!(
        command,
        CyberfabricCommand::Help(HelpArgs {
            command: HelpCommand::Schema(SchemaArgs {
                target: SchemaTarget::Config,
                section: Some("database".to_owned()),
            }),
        })
    );
}

#[test]
fn parses_help_schema_module() {
    let command = parse_command(&["cyberfabric", "help", "schema", "module"]);

    assert_eq!(
        command,
        CyberfabricCommand::Help(HelpArgs {
            command: HelpCommand::Schema(SchemaArgs {
                target: SchemaTarget::Module,
                section: None,
            }),
        })
    );
}

#[test]
fn parses_help_docs_with_query() {
    let command = parse_command(&["cyberfabric", "help", "docs", "tokio::sync"]);

    assert_eq!(
        command,
        CyberfabricCommand::Help(HelpArgs {
            command: HelpCommand::Docs(cyberware_cli_core::docs::DocsArgs {
                path: PathBuf::from("."),
                registry: cyberware_cli_core::common::Registry::CratesIo,
                verbose: false,
                libs: false,
                version: None,
                clean: false,
                query: Some("tokio::sync".to_owned()),
            }),
        })
    );
}

#[test]
fn parses_help_docs_with_all_flags() {
    let command = parse_command(&[
        "cyberfabric",
        "help",
        "docs",
        "-p",
        "/tmp/ws",
        "-v",
        "--libs",
        "--version",
        "1.0.0",
        "--clean",
        "cf-modkit",
    ]);

    assert_eq!(
        command,
        CyberfabricCommand::Help(HelpArgs {
            command: HelpCommand::Docs(cyberware_cli_core::docs::DocsArgs {
                path: PathBuf::from("/tmp/ws"),
                registry: cyberware_cli_core::common::Registry::CratesIo,
                verbose: true,
                libs: true,
                version: Some(semver::Version::new(1, 0, 0)),
                clean: true,
                query: Some("cf-modkit".to_owned()),
            }),
        })
    );
}

#[test]
fn parses_help_topic_manifest() {
    let command = parse_command(&["cyberfabric", "help", "topic", "manifest"]);

    assert_eq!(
        command,
        CyberfabricCommand::Help(HelpArgs {
            command: HelpCommand::Topic(TopicArgs {
                topic: Topic::Manifest,
            }),
        })
    );
}

#[test]
fn parses_help_topic_module_refs() {
    let command = parse_command(&["cyberfabric", "help", "topic", "module-refs"]);

    assert_eq!(
        command,
        CyberfabricCommand::Help(HelpArgs {
            command: HelpCommand::Topic(TopicArgs {
                topic: Topic::ModuleRefs,
            }),
        })
    );
}

#[test]
fn parses_help_topic_generated_server() {
    let command = parse_command(&["cyberfabric", "help", "topic", "generated-server"]);

    assert_eq!(
        command,
        CyberfabricCommand::Help(HelpArgs {
            command: HelpCommand::Topic(TopicArgs {
                topic: Topic::GeneratedServer,
            }),
        })
    );
}

#[test]
fn parses_help_topic_fips() {
    let command = parse_command(&["cyberfabric", "help", "topic", "fips"]);

    assert_eq!(
        command,
        CyberfabricCommand::Help(HelpArgs {
            command: HelpCommand::Topic(TopicArgs { topic: Topic::Fips }),
        })
    );
}

#[test]
fn parses_help_topic_otel() {
    let command = parse_command(&["cyberfabric", "help", "topic", "otel"]);

    assert_eq!(
        command,
        CyberfabricCommand::Help(HelpArgs {
            command: HelpCommand::Topic(TopicArgs { topic: Topic::Otel }),
        })
    );
}

#[test]
fn help_schema_rejects_unknown_target() {
    assert_parse_error(
        &["cyberfabric", "help", "schema", "bogus"],
        clap::error::ErrorKind::InvalidValue,
    );
}

#[test]
fn help_topic_rejects_unknown_topic() {
    assert_parse_error(
        &["cyberfabric", "help", "topic", "bogus"],
        clap::error::ErrorKind::InvalidValue,
    );
}
