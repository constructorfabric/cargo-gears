...
impl From<ManifestArgs> for cargo_gears_core::manifest::ManifestParams {
    fn from(args: ManifestArgs) -> Self {
        ...
        let (path, command) = match args.command {
            ManifestCommand::Validate(sub) => (
                ...
                cargo_gears_core::manifest::ManifestCommand::Validate { format: sub.format },
            ),
            ManifestCommand::Ls(sub) => (
                sub.path.or(parent_path),
                cargo_gears_core::manifest::ManifestCommand::Ls { format: sub.format },
            ),
        };
        Self {
            path,
