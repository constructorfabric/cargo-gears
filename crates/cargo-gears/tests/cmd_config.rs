...
use cargo_gears_core::app_config::{DbConnConfig, DbEngineCfg, PoolCfg};
use cargo_gears_core::common::PathConfigParams;
...
fn parses_config_db_add_into_core_command() {
    ...
    assert_eq!(
        command,
        GearsCommand::Config(cargo_gears_core::config::ConfigParams {
            command: cargo_gears_core::config::ConfigCommand::Db(Box::new(
                cargo_gears_core::config::db::DbParams {
                    command: cargo_gears_core::config::db::DbCommand::Add(
                        cargo_gears_core::config::db::AddArgs {
                            path_config: PathConfigParams {
                                path: None,
                                config: Some(PathBuf::from("config.yml")),
                            },
