...
/// Check if an item has the `#[cf_gears_toolkit_macros::api_dto(...)]` attribute.
///
...
pub fn has_api_dto_attribute(item: &rustc_ast::Item) -> bool {
    for attr in &item.attrs {
        // Check for cf_gears_toolkit_macros::api_dto or just api_dto
        if let rustc_ast::AttrKind::Normal(attr_item) = &attr.kind {
            ...
            // Match: api_dto, cf_gears_toolkit_macros::api_dto
